...
export function ReviewBar({ run, busy, onRevise, onApproveCopy, onApproveAssets, onRetry, onCancel }: Props) {
  ...
  return <section className="review-bar"><div><b>{copyReview ? '文案审核' : assetReview ? '资产审核' : '运行状态'}</b><span>{run.error || (completed ? '发布包已导出' : run.current_step)}</span></div>
    {copyReview && <><input aria-label="修改指令" value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder="输入具体修改指令" /><button type="button" onClick={() => void onRevise(instructions.trim())} disabled={busy || !instructions.trim()}><RotateCcw size={16} />返修</button><button type="button" className="primary" onClick={() => void onApproveCopy()} disabled={busy}><Send size={16} />通过文案</button></>}
    {assetReview && <button type="button" className="primary" onClick={() => void onApproveAssets()} disabled={busy}><Check size={16} />通过资产并导出</button>}
    {run.status === 'failed' && <button type="button" onClick={() => void onRetry()} disabled={busy}><TimerReset size={16} />重试</button>}
    ...
    <a className={`download ${completed ? '' : 'disabled'}`} href={run.final_package?.zip_url ? api.url(run.final_package.zip_url) : undefined}><Download size={16} />下载发布包</a>
  </section>
}
