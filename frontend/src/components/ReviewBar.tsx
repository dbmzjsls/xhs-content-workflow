...
type Props = { run?: Run; busy: boolean; copyApprovalEnabled: boolean; onRevise: (instructions: string) => Promise<void>; onApproveCopy: () => Promise<void>; onApproveAssets: () => Promise<void>; onRetry: () => Promise<void>; onCancel: () => Promise<void> }
const active = new Set(['queued', 'running', 'image_queued', 'image_running'])

export function ReviewBar({ run, busy, copyApprovalEnabled, onRevise, onApproveCopy, onApproveAssets, onRetry, onCancel }: Props) {
